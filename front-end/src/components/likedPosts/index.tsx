...
import { likeApi, likedPostsApi, saveApi } from "@/api/postActions";
...
export default function LikedPosts() {
    const [likedPosts, setLikedPosts] = useState<AddPosteTypes[]>([]);
    const { userInfo } = useContext(shareInfo);

    /*---> Fetch All Posts <---*/
    const getLikedPosts = async () => {
        try {
            const response = await likedPostsApi();
    ...
    return <>
        <div className="w-full">
            {likedPosts && likedPosts?.length > 0 ? (
                likedPosts?.map((item: AddPosteTypes) => (
